...
def vectorize_input(batch, config, training=True, device=None):
    ...
    with torch.set_grad_enabled(training):
        ...
        if batch.has_sent2:
            ...
            example['context2_lens'] = context2_lens.to(
                device) if device else context2_lens
        return example
...
def prepare_datasets(config):
    ...
    if config['data_type'] == 'network':
        adj, features, labels, idx_train, idx_val, idx_test = network_data_utils.load_data(config['data_dir'], config['dataset_name'], knn_size=config.get('input_graph_knn_size', None), epsilon=config.get('input_graph_epsilon', None), knn_metric=config.get(
            'knn_metric', 'cosine'), prob_del_edge=config.get('prob_del_edge', None), prob_add_edge=config.get('prob_add_edge', None), seed=config.get('data_seed', config['seed']), sparse_init_adj=config.get('sparse_init_adj', False))
        ...
        adj, features, labels, idx_train, idx_val, idx_test = network_data_utils.load_dgl_graph(config['dgl_graph'],
                                                                                                knn_size=config.get(
                                                                                                    'input_graph_knn_size', None),
                                                                                                epsilon=config.get(
                                                                                                    'input_graph_epsilon', None),
                                                                                                knn_metric=config.get(
                                                                                                    'knn_metric', 'cosine'),
                                                                                                prob_del_edge=config.get(
                                                                                                    'prob_del_edge', None),
                                                                                                prob_add_edge=config.get(
                                                                                                    'prob_add_edge', None),
                                                                                                seed=config.get(
                                                                                                    'data_seed', config['seed']),
                                                                                                sparse_init_adj=config.get('sparse_init_adj', False))
        ...
        data_conf = uci_data_utils.UCI(seed=config.get(
            'data_seed', config['seed']), dataset_name=config['dataset_name'], n_train=config['n_train'], n_val=config['n_val'])
        adj, features, labels, idx_train, idx_val, idx_test = data_conf.load(data_dir=config.get(
            'data_dir', None), knn_size=config['input_graph_knn_size'], epsilon=config.get('input_graph_epsilon', None), knn_metric=config.get('knn_metric', 'cosine'))
...
class DataStream(object):
    def __init__(self, all_instances, word_vocab, config=None,
        ...
        if isSort:
            all_instances = sorted(all_instances, key=lambda instance: [
                                   len(x) for x in instance[:-1]])  # the last element is label
        else:
        ...
        batch_spans = padding_utils.make_batches(
...
class InstanceBatch(object):
    def __init__(self, instances, config, word_vocab):
        ...
        if len(instances[0]) == 2:
            ...
            raise RuntimeError(
                '{} elements per example, should be 2 or 3'.format(len(instances[0])))
        ...
        for instance in self.instances:
            ...
            self.sent1_word.append([word_vocab.getIndex(word)
                                   for word in sent1_cut])
            self.sent1_length.append(len(sent1_cut))
            if self.has_sent2:
                sent2_cut = instance[1][: config.get('max_seq_len', None)]
                self.sent2_word.append(
                    [word_vocab.getIndex(word) for word in sent2_cut])
        ...
        if self.has_sent2:
            self.sent2_word = padding_utils.pad_2d_vals_no_size(
                self.sent2_word)
