...
def load_data(data_dir, dataset_str, knn_size=None, epsilon=None, knn_metric='cosine', prob_del_edge=None, prob_add_edge=None, seed=1234, sparse_init_adj=False):
    ...
    if dataset_str.startswith('ogbn'):  # Open Graph Benchmark datasets
        ...
        assert adj.diagonal().sum() == 0 and adj.max() <= 1 and (
            adj != adj.transpose()).sum() == 0
    ...
    if not knn_size is None:
        ...
        adj = kneighbors_graph(
            features, knn_size, metric=knn_metric, include_self=True)
        ...
        feature_norm = features.div(torch.norm(
...
def load_dgl_graph(g, knn_size=None, epsilon=None, knn_metric='cosine', prob_del_edge=None, prob_add_edge=None, seed=1234, sparse_init_adj=False):

    assert (knn_size is None) or (epsilon is None)

    features = g.ndata['features']
    labels = g.ndata['labels']
    idx_train, temp = train_test_split(g.nodes(), train_size=0.6)
    idx_val, idx_test = train_test_split(temp, train_size=0.5)
    ...
    if not knn_size is None:
        ...
        print('something went wrong @ network_data/data_utils.py')

    # print(torch.count_nonzero(adj_norm))
    print(torch.tensor(len(torch.nonzero(adj_norm))))
    return adj_norm, features, labels, idx_train, idx_val, idx_test
