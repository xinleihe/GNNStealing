...
class Config:
    ...
    def __str__(self):
        def _sting_kw(k, v): return '{}={}'.format(k, v)
    ...
    def grid(cls, **kwargs):
        ...
        sin = OrderedDict(
            {k: v for k, v in kwargs.items() if isinstance(v, list)})
...
class ConfigData(Config):
    ...
    def load(self):
        res = eval(self.f1)(seed=self.seed,
                            dataset_name=self.dataset_name, **self.kwargs_f1)
        if self.f2:
...
class UCI(ConfigData):
    ...
    def load(self, data_dir=None, knn_size=None, epsilon=None, knn_metric='cosine'):
        ...
        if self.dataset_name == 'iris':
            ...
            data = pickle.load(
                open(os.path.join(data_dir, '20news10.pkl'), 'rb'))
